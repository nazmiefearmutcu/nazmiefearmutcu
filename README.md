...
| --- | --- | --- |
| **[Prizma](https://github.com/nazmiefearmutcu/Prizma)** | Two research threads held to pre-registered falsifiable bars with adversarial referee audits: a parameter-free quadratic delta-state sequence mixer (an efficient-attention candidate) and backprop-free, fully-local continual learning. The bar is committed to git before the run — so a missed bar gets reported, not quietly moved. | `Python` · `PyTorch` |
...
| **[dive-into-crypto](https://github.com/nazmiefearmutcu/dive-into-crypto)** | Points to the handful of Binance USDT-perp coins where 15 indicators agree across all 12 timeframes at once — not one indicator's lucky guess — and surfaces one consensus verdict per symbol, filtered by whale-microstructure divergence. Native Android, runs on your device, fully read-only: it scans and ranks, it never places an order. | `Kotlin` · `Compose` |
